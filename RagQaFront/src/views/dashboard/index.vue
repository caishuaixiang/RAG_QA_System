...
const loadStats = async () => {
  try {
    ...
    if (userStore.userInfo?.id) {
      const statsRes = await userApi.getUserStats(userStore.userInfo.id)
      if (statsRes.code === 200) {
        stats.knowledgeCount = statsRes.data?.knowledgeCount || 0
        stats.documentCount = statsRes.data?.documentCount || 0
        stats.qaCount = statsRes.data?.qaCount || 0
        stats.chunkCount = statsRes.data?.chunkCount || 0
      }
    }
  } catch (error) {
